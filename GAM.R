...
# Introducing non-linear dependencies
...
for (k in 1:K) {
  ...
  for (i in 1:n) {
    ...
    model <- gam(formula = Formula, data = cases_trn, family = binomial(link = "logit"))
  ...
  models[[k]] <- gam(formula = Formula, data = cases_trn, family = binomial(link = "logit"))
...
save(recap_valid, file = "C:/Users/fiko1/Desktop/UWr/Z2024/Wierzytelności masowe/Projekt/lab06/models/models_gam.RData")
